$(document).ready( function () {
    ...
    $('#DataTable').DataTable({
        ...
        "columnDefs": [ { "targets": [ 0, 1, 9 , 10],  "visible": false  }],
        "initComplete": function() {
            // data loaded & sorted - now show table
            $("#loading").hide();
            $("#DataTable").show();
      ...
      $('#yearfilters').on('click', 'label', function() {
        ...
        if (year == "All") {
          ...
          // call search api (regex option  to catch 4 or 5) and redraw table
          $('#DataTable').DataTable().column(12).search('4|5|6', true, false).draw();
        } else {
          //console.log ("Search " + year);
          // call search api and redraw table
          $('#DataTable').DataTable().column(12).search(year).draw();
        }
      });
      ...
      $('#courseSelect').on('change',  function() {
        ...
        // add exact match like https://datatables.net/examples/api/multi_filter_select.html
        //$('#DataTable').DataTable().column(13).search(course).draw();
        $('#DataTable').DataTable().column(13).search(course ? '^'+course+'$' : '',true,false).draw();
      });
